...
const SERVICES = [
  {
    icon: Globe,
    label: "Web Design",
    desc: "Conversion-focused WordPress websites",
    href: "/services/web-design",
  },
  {
    icon: Shield,
    label: "Website Maintenance",
    desc: "Security, backups and monthly updates",
    href: "/services/website-maintenance",
  },
  {
    icon: TrendingUp,
    label: "Conversion Optimisation",
    desc: "More enquiries from existing traffic",
    href: "/services/conversion-optimisation",
  },
  {
    icon: Layers,
    label: "Brand Identity",
    desc: "Logo, colours, typography & guidelines",
    href: "/services/branding",
  },
  {
    icon: PenTool,
    label: "Graphic Design",
    desc: "Print, social media & marketing design",
...
const NAV_LINKS_LEFT  = [{ label: "Projects", href: "/projects" }];
const NAV_LINKS_RIGHT = [{ label: "Packages", href: "/packages" }, { label: "About", href: "/about" }];
...
export function Navigation() {
  ...
  const openServices  = () => { setServicesOpen(v => !v); setResourcesOpen(false); };
  const openResources = () => { setResourcesOpen(v => !v); setServicesOpen(false); };
  const close = () => { setMenuOpen(false); setServicesOpen(false); setResourcesOpen(false); };

  const DropdownLink = ({ item, onClose }: { item: typeof SERVICES[0]; onClose: () => void }) => {
    ...
    return (
      ...
      >
        <div className="w-9 h-9 rounded-xl bg-foreground/5 group-hover:bg-foreground/8 flex items-center justify-center shrink-0 transition-colors">
          <Icon size={14} className="text-foreground/45 group-hover:text-foreground/70 transition-colors" />
        </div>
        <div>
          <p className="text-[13px] font-semibold leading-none mb-1 text-foreground group-hover:text-accent transition-colors">{item.label}</p>
          ...
          <nav aria-label="Main navigation" className="hidden lg:flex items-center justify-center gap-8">
            ...
            <div className="relative" ref={servicesRef}>
              <button
                ...
                className={`flex items-center gap-1 text-[13px] font-medium transition-colors duration-200 py-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 rounded-sm whitespace-nowrap ${servicesOpen ? "text-foreground" : "text-foreground/50 hover:text-foreground"}`}
                aria-expanded={servicesOpen}
                aria-haspopup="true"
              >
                Services
                <ChevronDown size={12} strokeWidth={2} className={`transition-transform duration-250 ${servicesOpen ? "rotate-180 text-accent" : ""}`} />
              </button>
              ...
              <AnimatePresence>
                {servicesOpen && (
                  <motion.div
                    ...
                    className="absolute top-[calc(100%+14px)] left-1/2 -translate-x-1/2 w-[300px] z-50"
                    style={{ filter: "drop-shadow(0 8px 24px rgba(0,0,0,0.10))" }}
                  >
                    <div aria-hidden className="absolute -top-[7px] left-1/2 -translate-x-1/2 w-3 h-3 bg-white border-l border-t border-border rotate-45" />
                    <div className="bg-white border border-border rounded-2xl overflow-hidden">
                      <div className="h-[3px]" style={{ background: "linear-gradient(90deg, hsl(22 88% 48%) 0%, hsl(22 88% 58% / 0.25) 100%)" }} />
                      <div className="p-2">
                        {SERVICES.map(item => (
                          <DropdownLink key={item.href} item={item} onClose={() => setServicesOpen(false)} />
                        ))}
                        <div className="mx-3.5 mt-2 pt-2 border-t border-border">
                          <Link
                            href="/services"
                            onClick={() => setServicesOpen(false)}
                            className="flex items-center justify-between py-2 text-[12px] font-semibold text-foreground/40 hover:text-accent transition-colors"
                          >
            ...
            <div className="relative" ref={resourcesRef}>
              ...
              >
                Resources
                <ChevronDown size={12} strokeWidth={2} className={`transition-transform duration-250 ${resourcesOpen ? "rotate-180 text-accent" : ""}`} />
              </button>
                        ...
                        {RESOURCES.map(item => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <div>
                                <p className={`text-[13px] font-semibold leading-none mb-1 transition-colors ${item.accent ? "text-accent" : "text-foreground group-hover:text-accent"}`}>{item.label}</p>
                                <p className="text-[11px] text-foreground/40 leading-snug">{item.desc}</p>
                              </div>
                            </Link>
                          );
          ...
          <div className="flex items-center justify-end gap-3">
            ...
            >
              <span className="flex flex-col gap-[5px] w-[18px]">
                <motion.span className="block h-[1.5px] bg-foreground origin-center rounded-full" animate={menuOpen ? { rotate: 45, y: 6.5 } : { rotate: 0, y: 0 }} transition={{ duration: 0.22 }} />
                <motion.span className="block h-[1.5px] bg-foreground rounded-full" animate={menuOpen ? { opacity: 0, scaleX: 0 } : { opacity: 1, scaleX: 1 }} transition={{ duration: 0.18 }} />
                <motion.span className="block h-[1.5px] bg-foreground origin-center rounded-full" style={{ width: "12px" }} animate={menuOpen ? { rotate: -45, y: -6.5, width: "18px" } : { rotate: 0, y: 0, width: "12px" }} transition={{ duration: 0.22 }} />
              </span>
            </button>
          </div>
          ...
          >
            <nav aria-label="Mobile navigation" className="flex-1 px-5 md:px-8 pt-3 overflow-y-auto">
              ...
              {NAV_LINKS_LEFT.map((link, i) => (
                ...
                >
                  <span className="text-2xl font-semibold tracking-tight text-foreground group-hover:text-accent transition-colors duration-150">{link.label}</span>
              ...
              >
                <div className="flex items-center justify-between py-[18px]">
                  <span className="text-2xl font-semibold tracking-tight text-foreground/35">Services</span>
                </div>
                <div className="flex flex-col gap-1 pb-4 -mt-1">
                  {SERVICES.map(item => {
                    ...
                    return (
                      ...
                      >
                        <div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0 bg-foreground/5">
                          <Icon size={13} className="text-foreground/40" />
                        </div>
                        <div>
                          <p className="text-[14px] font-semibold leading-none mb-0.5 text-foreground">{item.label}</p>
                  ...
                  <Link
                    href="/services"
                    onClick={close}
                    className="flex items-center gap-2 px-3 py-2 text-[13px] font-semibold text-foreground/40 hover:text-accent transition-colors"
                  >
                    All services <ArrowUpRight size={11} />
                  </Link>
                </div>
              ...
              {NAV_LINKS_RIGHT.map((link, i) => (
                <motion.a
                  ...
                  initial={{ opacity: 0, x: -10 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.22, delay: 0.14 + i * 0.05 }}
                  className="flex items-center justify-between py-[18px] border-b border-foreground/6 group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2"
                >
                  <span className="text-2xl font-semibold tracking-tight text-foreground group-hover:text-accent transition-colors duration-150">{link.label}</span>
                  <ArrowUpRight size={14} className="text-foreground/20 group-hover:text-accent transition-colors duration-150" />
            ...
            >
              ...
              >
                Free Website Review <ArrowUpRight size={14} />
              </a>
